...
from trivena_meet.utils.sfu_config import get_sfu_config
from trivena_meet.utils.user import (
	get_guest_session,
	get_user_info,
	set_guest_session,
	validate_guest_name,
...
if TYPE_CHECKING:
	from trivena_meet.meet.doctype.sae_meeting.sae_meeting import SaeMeeting
...
def _generate_sfu_token(
	...
	**extra,
	...
	secret = sfu_config.get("sfu_secret") or trivena.conf.get("secret_key")
	if not secret:
		trivena.throw(_("SFU secret not configured"))

	now = int(time.time())
	payload = {
		"user_id": user_id,
		"meeting_id": meeting_id,
		"site": getattr(trivena.local, "site", None),
		"scope": scope,
		"exp": now + expires_in,
		"iat": now,
		**extra,
...
def _get_codec_strategy() -> str:
	return trivena.get_cached_doc("Sae Settings").codec_strategy or "svc"
...
def create(meeting_type: str = "open", allow_guest: bool = True) -> str:
	...
	global_settings = trivena.get_cached_doc("Sae Settings")
	if not global_settings.allow_guest:
		allow_guest = False

	meeting: SaeMeeting = trivena.get_doc(
		{
			"doctype": "Sae Meeting",
...
def get_sfu_connection_details(meeting_id: str) -> dict:
	...
	"""
	meeting: SaeMeeting = trivena.get_doc("Sae Meeting", meeting_id)
	user = trivena.session.user

	if meeting.is_user_banned(user):
		trivena.throw(_("You are banned from this meeting"), trivena.PermissionError)

	if user not in meeting.get_members():
		trivena.throw(_("Not a meeting member"), trivena.PermissionError)

	if not meeting.can_join(user):
		trivena.throw(_("Access denied"), trivena.PermissionError)

	sfu_config = get_sfu_config()

	user_fullname, user_avatar = trivena.db.get_value("User", user, ["full_name", "user_image"]) or (
		user,
		None,
	)
...
def join_meeting(meeting_id: str) -> dict:
	...
	if meeting.is_user_banned(trivena.session.user):
		trivena.throw(_("You are banned from this meeting"), trivena.PermissionError)

	if meeting.can_join(trivena.session.user):
		result = meeting.join(trivena.session.user)

		if isinstance(result, dict):
			if result.get("status") == "waiting_for_approval":
				sfu_config = get_sfu_config()

				user_fullname, user_avatar = trivena.db.get_value(
					"User", trivena.session.user, ["full_name", "user_image"]
				) or (trivena.session.user, None)

				lobby_token = _generate_sfu_token(
					user_id=trivena.session.user,
					meeting_id=meeting_id,
				...
				is_host = meeting.owner == trivena.session.user
				is_cohost = meeting.is_host_or_cohost(trivena.session.user) and not is_host
				return {
					"status": "joined",
					"meeting_id": meeting_id,
...
def get_waiting_room(meeting_id: str) -> dict:
	...
	if not meeting.is_host_or_cohost(trivena.session.user):
		trivena.throw(_("Access denied"))
...
def refresh_sfu_token(meeting_id: str) -> dict:
	...
	if trivena.session.user not in meeting.get_members():
		trivena.throw(_("Not a meeting member"))
...
def get_sfu_presence_preview_token(meeting_id: str) -> dict:
	...
	try:
		meeting: SaeMeeting = trivena.get_doc("Sae Meeting", meeting_id)
	except trivena.DoesNotExistError:
		trivena.throw(_("Meeting not found"))

	if meeting.is_user_banned(trivena.session.user):
		trivena.throw(_("You are banned from this meeting"), trivena.PermissionError)

	if not meeting.can_join(trivena.session.user):
		trivena.throw(_("Access denied"), trivena.PermissionError)
...
def join_meeting_as_guest(meeting_id: str, guest_name: str, guest_id: str | None = None) -> dict:
	...
	if not is_valid:
		trivena.throw(_(error_message))

	if not trivena.db.exists("Sae Meeting", meeting_id):
		trivena.throw(_("Meeting not found"))

	meeting = trivena.get_doc("Sae Meeting", meeting_id)

	global_settings = trivena.get_cached_doc("Sae Settings")
	if not global_settings.allow_guest or not meeting.allow_guest:
		trivena.throw(_("Guests are not allowed in this meeting"))
...
def get_approved_guest_connection_details(meeting_id: str, guest_id: str) -> dict:
	...
	session_data = get_guest_session(guest_id)
	if not session_data:
		trivena.throw(_("Guest session not found or expired"))
	...
	if not meeting.is_user_approved(guest_id):
		trivena.throw(_("Guest not approved"))
...
def get_guest_sfu_connection_details(meeting_id: str, guest_token: str) -> dict:
	...
	try:
		...
		trivena.throw(_("Guest token has expired"))
	except jwt.InvalidTokenError:
		trivena.throw(_("Invalid guest token"))

	if not decoded.get("is_guest"):
		trivena.throw(_("Not a guest token"))

	if decoded.get("meeting_id") != meeting_id:
		trivena.throw(_("Token meeting ID does not match"))
...
def promote_to_cohost(meeting_id: str, user_id: str) -> dict:
	...
	"""
	meeting: SaeMeeting = trivena.get_doc("Sae Meeting", meeting_id)
	return meeting.promote_to_cohost(trivena.session.user, user_id)
...
def check_meeting_access(meeting_id: str) -> dict:
	...
	try:
		meeting: SaeMeeting = trivena.get_doc("Sae Meeting", meeting_id)
		settings = trivena.get_cached_doc("Sae Settings")
