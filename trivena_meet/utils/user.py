...
from trivena_framework.core.doctype.user.user import User
...
def get_user_info(user_id: str) -> dict | None:
	...
	user_info = trivena.db.get_value("User", user_id, ["full_name", "user_image"], as_dict=True)
...
def get_guest_session(guest_id: str) -> dict | None:
	...
	session_data = trivena.cache.get_value(cache_key)
...
def set_guest_session(guest_id: str, session_data: dict, ttl: int = 86400) -> None:
	...
	cache_key = f"guest_session:{guest_id}"
	trivena.cache.set_value(cache_key, session_data, expires_in_sec=ttl)
