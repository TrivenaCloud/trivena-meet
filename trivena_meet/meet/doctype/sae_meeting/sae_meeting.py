...
class SaeMeeting(Document):
	...
	if TYPE_CHECKING:
		...
		from trivena_meet.meet.doctype.sae_meeting_user.sae_meeting_user import SaeMeetingUser
	...
	def after_insert(self):
		self.join(trivena.session.user)
	...
	def approve_user(self, user):
		...
		if not self.is_host_or_cohost(trivena.session.user):
			trivena.throw(_("Only hosts and co-hosts can approve join requests"))

		waiting_users = self.get_waiting_room()
		if user not in waiting_users:
			trivena.throw("User is not in waiting room")
		...
		trivena.publish_realtime(
			...
			user=user,
			message={"meeting": self.name, "user": user, "approved_by": trivena.session.user},
		)
	...
	def reject_user(self, user, rejected_by=None):
		...
		if not rejected_by:
			rejected_by = trivena.session.user

		if not self.is_host_or_cohost(rejected_by):
			trivena.throw(_("Only hosts and co-hosts can reject join requests"))
	...
	def validate_can_promote_to_cohost(self, user: str, target_user: str) -> None:
		...
		if user != self.owner:
			trivena.throw(_("Only the meeting host can promote users to co-host"))

		if target_user.startswith("guest_"):
			trivena.throw(_("Guests cannot be promoted to co-host"))

		if self.is_host_or_cohost(target_user):
			trivena.throw(_("User is already a host or co-host"))

		if target_user not in self.get_members():
			trivena.throw(_("User is not currently in the meeting"))
	...
	def validate_guest_id(self, guest_id: str):
		if not guest_id or not isinstance(guest_id, str):
			trivena.throw(_("Invalid guest ID"))

		if not guest_id.startswith("guest_"):
			trivena.throw(_("Invalid guest ID format"))

		if len(guest_id) < 7:
			trivena.throw(_("Invalid guest ID format"))

		if self.is_user_banned(guest_id):
			trivena.throw(_("Guest is banned from this meeting"))
	...
	def update_settings(
		...
		if not self.is_host_or_cohost(trivena.session.user):
			trivena.throw(_("Only the meeting host or co-host can update settings"))
